...
export async function createLiveAuction(auctioneerid:string, auctionId:string) {
  const token = getToken();
    if (!token) {
      throw new Error("No se encontró el token de autenticación.");
    }
  ...
  try{
    const response = await fetch(`http://localhost:5015/api/1.0/live/${auctionId}/${auctioneerid}`, 
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || "Error al crear remate en vivo"
      );
    }
    ...
    console.error("Error al crear remate en vivo: ", err);
    throw new Error("Error al crear remate en vivo");
  }
}
